...
internal fun PainAndGain.creepTurn(creep: Creep, ctx: Ctx, t: ArmyTick) {
    with(t) {
        ...
        val massKite: Creep? = if ((!support) && (enemyMassedNow) && !stalled &&
...
internal fun PainAndGain.armyTargets(ctx: Ctx, seg: ArmyTargetsIn): ArmyTargetsOut = with(seg) {
    ...
    // НИКОГО НЕ ПРОБИВАЕМ (v423): лечение перекрывает наш огонь на КАЖДОЙ цели пула. В этом состоянии ярус роли не
    // решает ничего — части не снимаются ни с кого, — и выбор отдаётся сходимости; считается один раз за тик
    val nobodyKillable = USE_FOCUS_GUNS_FIRST && focusPool.isNotEmpty() && focusPool.all { killTicks(it).isInfinite() }
    ...
    val focusCmp = compareBy<Creep> { if (it.hits <= fireAvailableAt(it) * InfluenceMap.takenOf(it)) 1 else 0 }
        ...
        // ...И СХОДИМОСТЬ СТАНОВИТСЯ ВЫШЕ «СТРЕЛКИ ПЕРВЫМИ» (v423). Оба яруса уже здесь, менялся только их ПОРЯДОК:
        // v60 ставит стрелка выше всех, а v70 («сколько наших стволов достаёт цель ПРЯМО СЕЙЧАС») стоял под ним и
        // потому решал лишь споры внутри роли. Чистая пара 16 игр против MetalicaX#13 (7-9, одна версия) вычла всю
        // симметрию матчапа и оставила ровно одну величину, которая принадлежит НАМ: выстрелов в одну цель за тик у
        // нас 2,8 ± 0,4 в победах против 2,2 ± 0,5 в поражениях, тогда как У НЕГО она постоянна (2,2 и 2,6), и знак
        // разности «наш conc минус его» и есть исход — +0,69 ± 0,52 против −0,34 ± 0,48, разрез по +0,30 даёт
        // 5 побед из 6 против 1 из 9. Всё остальное оказалось совместным или неразличимым: он бьёт БЛИЖАЙШЕГО в
        // 0,78 против 0,75, доля его выстрелов в наших стрелков одинакова (0,48 против 0,51), коридор до стрелка
        // открыт в 0,87 против 0,88, экранирование мили 0,31 против 0,32, а дистанция встречи зеркальна (наш мили
        // до его ближайшего 1,95 против 1,35 — и его мили до нашего 1,83 против 1,39).
        // ⚠️ Это НЕ v395 (отвергнут, 3-13): там ярус считал стволы, которые достанут цель ЗА ШАГ, и требовал от
        // расстановки шага, которого она не делает. Здесь мера мгновенная — цель выбирается среди тех, кого мы уже
        // достаём, и шага ни от кого не требует. И не v394: лекарь не трогается, ярусы выше остаются как есть
        // ...НО ТОЛЬКО КОГДА ПРОБИТЬ НЕКОГО. Первая редакция подняла сходимость над ролью безусловно, и гейт назвал
        // цену сразу: `match32:screen+flagless` из «недосягаемый отрыв на t=1133, счёт 21 962 : 285» превратился в
        // «наша армия уничтожена на t=808, счёт 4 662 : 2 901». Механизм очевиден и верен: больше всего наших
        // стволов достаёт ЭКРАН, и правило без условия сосредотачивало огонь на мясе вместо стрелков. Ярус «стрелки
        // первыми» (v60) защищает ровно от этого и снимать его нельзя. Условие берётся из самого предмета: когда
        // НИ ОДНА цель не пробиваема (лечение везде перекрывает наш огонь — `killTicks` бесконечен у всех), выбор
        // «стрелок или нет» не решает ничего, потому что мы всё равно не снимаем ни одной части; тогда и только
        // тогда важнее собрать стволы. Это ровно то состояние, которое измерено в поражениях: с него не снимается
        // НИ ОДНОЙ части за бой (0 из 15 матчей к контакту+20)
        .thenBy { if (USE_FOCUS_GUNS_FIRST && nobodyKillable) gunsAt(it) else 0 }
        ...
        .thenBy { if (USE_FOCUS_GUNS_FIRST) 0 else gunsAt(it) }
