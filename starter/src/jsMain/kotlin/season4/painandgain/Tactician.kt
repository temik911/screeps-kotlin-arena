...
internal fun PainAndGain.armyTargets(ctx: Ctx, seg: ArmyTargetsIn): ArmyTargetsOut = with(seg) {
    ...
    /** ЦЕЛЬ, ДО КОТОРОЙ ДОТЯНУЛСЯ НАШ МИЛИ (v429): к её залпу прибавляются 240 удара, и только такая цель пробивается. */
    fun meleeAt(e: Creep?) = e != null && combatArmy.any { hasMelee(it) && !hasRanged(it) && it.getRangeTo(e) <= 1 }
    val focusCmp = compareBy<Creep> { if (it.hits <= fireAvailableAt(it) * InfluenceMap.takenOf(it)) 1 else 0 }
        ...
        // ...И ВНУТРИ РОЛИ ВЫШЕ ТА ЦЕЛЬ, ДО КОТОРОЙ ДОТЯНУЛСЯ НАШ МИЛИ (v429). Основание арифметическое, а не из
        // разреза реплеев: его лечение на одну цель 129–147 в тик, наш выстрел 60. Залп трёх стрелков (180) даёт
        // ЧИСТЫМИ 33–51 — такую цель не добить за время боя; те же три ствола плюс удар мили (240) дают 420, то
        // есть 273–291 чистыми, ВШЕСТЕРО быстрее. Значит пробиваема ровно та цель, к которой дотянулся наш мили, и
        // ярус ставится ПОД ролью (v60 «стрелки первыми» остаётся старше): выбор идёт не «кого бить вместо
        // стрелка», а «какого из стрелков бить». Это и защищает от ловушки v423, где сходимость, поднятая НАД
        // ролью, собрала огонь на экране и уронила гейт (match32: 21 962 : 285 превратилось в гибель армии)
        .thenBy { if (USE_FOCUS_MELEE_REACH && meleeAt(it)) 1 else 0 }
        .thenBy { gunsAt(it) }
