...
internal class Turn(val creep: Creep, val ctx: Ctx, val t: ArmyTick) {
    ...
    // ⚠️ ...И МЕСТНЫЙ ПЕРЕВЕС ЗДЕСЬ ЗНАЧИТ ТО ЖЕ, ЧТО У `spotNow` (v526, см. USE_COVER_NEEDS_KILLABLE). Восьмой
    // случай класса: одни и те же ворота «есть местный перевес у цели» стоят в двух местах, и только в одном из
    // них спрошена ДОБИВАЕМОСТЬ. Довод записан у `spotNow` дословно — «очаг у трёх его лекарей не очаг»: перевес
    // по стволам ничего не значит, если цель лечат быстрее, чем мы её бьём, а наш мили платит за вход всеми
    // восемью частями ATTACK (800 хитов из 1600) под залп всего, что достаёт
    fun covered(e: Creep) =  holdMelee ||
        // ...И ТОЛЬКО ПРОТИВ СОМКНУТОГО (сужение по гейту): против РОССЫПИ добиваемость почти всегда бесконечна —
        // цель уходит, — и требование запирает преследование там, где догонять и надо. Гейт назвал цену сразу:
        // match30:scatter из 24278 в свою пользу превращался в 20875:24278. Признак тот же, что у v220
        (spotEdgeAt(e) >= PUSH_RATIO && (!USE_COVER_NEEDS_KILLABLE || !meas.forces.enemyMassedNow ||
            !targ.focus.killTicks(e).isInfinite())) ||
        (guards(e)) ||
        strat.inp.combatArmy.count { it.id != creep.id && hasRanged(it) && getRange(it, e) <= RANGED_RANGE + 1 } >= MELEE_COVER ||
        ctx.army.any { a -> a.id != creep.id && getRange(e, a) <= 1 }
