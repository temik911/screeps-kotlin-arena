...
internal class Ctx() {
    ...
    init { bfsMaxCost = maxOf(bfsMaxCost, bfsCost) }
    init { bfsThisTick = 0 }
    init { bfsCost = 0.0 }
    private val nowT = getTicks()
    private val dead = Memory.flowCacheTick.filterValues { nowT - it > FLOW_KEEP }.keys.toList()
    init { for (k in dead) { Memory.flowCache.remove(k); Memory.flowCacheTick.remove(k) } }
    init { avoidCellsCache = null }
    ...
    init { if (homePos == null && active.isNotEmpty()) mirrorTL = (active.sumOf { it.x } + active.sumOf { it.y }) < 98 * active.size }
    init { if (homePos == null && active.isNotEmpty()) homePos = centroidOf(active) }
    init { if (enemyHomePos == null && enemyCreeps.isNotEmpty()) enemyHomePos = centroidOf(enemyCreeps) }
    ...
    private val walls = getObjectsByPrototype(StructureWall::class).filter { it.exists }
    private val ramparts = getObjectsByPrototype(StructureRampart::class).filter { it.exists }
    private val spawns = getObjectsByPrototype(StructureSpawn::class).filter { it.exists }
    val blocked: List<Position> = walls + ramparts.filter { it.my != true } + spawns + immobile
    private val blockedForEnemy: List<Position> = walls + ramparts.filter { it.my != false } + spawns

    init { InfluenceMap.setProtectedCells(ramparts.filter { it.my == true }.mapTo(HashSet()) { it.key }) }
    init { InfluenceMap.setEnemyBlocked(blockedForEnemy.mapTo(HashSet()) { it.key }) }
    ...
    init { InfluenceMap.buildFields(active, enemyCreeps) }
    ...
    private val notOurs = flags.filter { !it.ours }
    ...
    private val blockSig = blocked.sumOf { it.key + 1 } * 31 + flagBlocked.sumOf { it.key + 1 }
    // смена препятствий: поля не удаляются, а помечаются устаревшими — сверх бюджета (см. BFS_BUDGET) идут как есть
    init { if (blockSig != flowSig) { for (k in Memory.flowCacheTick.keys.toList()) Memory.flowCacheTick[k] = -1000; flowSig = blockSig } }
    /** Опасность + НЕ НАШИ флаги как стены: путь без назначения на флаг не ступает. */
    val dangerMatrix = rawDanger.clone()
    init { for (c in flagCells) dangerMatrix.set(c / 100, c % 100, 255) }
    ...
    init { DistanceMap.syncWalls(walls.size) }
    init { DistanceMap.ensureBuilt(home, enemyHome) }
    init { cpuMark("built") }
    ...
    val threats = combatEnemies.filter { threatening(it, enemyCreeps) }
    ...
    init { cpuMark("arrival") }
}

// ==================== флаги и эффекты ====================

internal fun collectFlags(myCreeps: List<Creep>, enemyCreeps: List<Creep>, combatEnemies: List<Creep>): List<FlagInfo> {
    ...
    for (fi in result) {
        ...
        if (prev != null && prev != owner) WorldState.flagFlipNow = true       // событие для стратега (v242)
...
internal fun accountScore(flags: List<FlagInfo>) {
    val cap = num(MAX_SCORE_PER_TICK.asDynamic(), Double.MAX_VALUE)
    WorldState.ourRate = minOf(cap, oursOf(flags).sumOf { it.score }.toDouble()).toInt()
    WorldState.enemyRate = minOf(cap, flags.filter { it.theirs }.sumOf { it.score }.toDouble()).toInt()
    ourScore += WorldState.ourRate
    enemyScore += WorldState.enemyRate
    val remaining = maxOf(0, arenaInfo.ticksLimit - getTicks())
    WorldState.behindOnScore = ourScore + WorldState.ourRate * remaining < enemyScore + WorldState.enemyRate * remaining
    behindTicks = if (WorldState.behindOnScore) behindTicks + 1 else 0
    ...
    if (getTicks() == 100) race100 = "${WorldState.ourRate}:${WorldState.enemyRate}"
    if (getTicks() == 200) race200 = "${WorldState.ourRate}:${WorldState.enemyRate}"
    if (DEBUG_LOG && getTicks() % 100 == 0) println("score t=${getTicks()}: our=${ourScore.toInt()} (+${WorldState.ourRate}/t) enemy=${enemyScore.toInt()} (+${WorldState.enemyRate}/t) behind=${WorldState.behindOnScore} lead=${(ourScore - enemyScore).toInt()} maxSwing=${(cap * remaining).toInt()}")
}
...
internal fun heldFlag(ctx: Ctx, c: Creep): FlagInfo? =
    ctx.flags.firstOrNull { it.ours && it.pos.x == c.x && it.pos.y == c.y }
        ?.takeIf { f -> Signals.groupSafe || ctx.enemyCreeps.any { getRange(it, f.pos) <= HOLD_WATCH } }
...
/** УКЛОНЕНИЕ СКАУТА ПО ОБЛАСТИ ПРЕСЛЕДУЕМОГО (v576, см. USE_SCOUT_MARGIN_EVADE). Для каждой клетки — когда его стволы смогут
...
internal object ScoutEvade {
    ...
    private val fire = IntArray(N)
    private val walkDist = IntArray(N)
    private val parent = IntArray(N)
    private val queue = IntArray(N)
    private val nodeCell = IntArray(N * 9)
    private val nodeNext = IntArray(N * 9)
    private var fireTick = -1
    // область шага (v596): метка поколения вместо заливки массива на каждый поиск
    ...
    private fun hunterTimes(hunters: List<Creep>) {
        ...
        val heads = IntArray(sc + 1) { -1 }
        var pool = 0
        hunt.fill(INF)
        var pending = 0
        for (h in hunters) {
            val k = key(h.x, h.y)
            if (hunt[k] == 0) continue
            hunt[k] = 0
            nodeCell[pool] = k; nodeNext[pool] = heads[0]; heads[0] = pool; pool++; pending++
        }
        var current = 0
        while (pending > 0) {
            val b = current % (sc + 1)
            while (heads[b] >= 0) {
                ...
                for (dx in -1..1) for (dy in -1..1) {
                    ...
                    val n = key(nx, ny)
                    if (wall[n]) continue
                    val next = current + if (swamp[n]) sc else 1
                    if (next < hunt[n] && pool < nodeCell.size) {
                        hunt[n] = next
                        val nb = next % (sc + 1)
    ...
    /** Срок обстрела клетки: минимум его времени по окну дальности выстрела (разделимый минимум — строки, затем столбцы). */
    private fun fireTimes(reach: Int) {
        for (x in 0 until 100) for (y in 0 until 100) {
            var m = INF
            for (d in -reach..reach) { val xx = x + d; if (xx in 0..99) { val v = hunt[key(xx, y)]; if (v < m) m = v } }
            rows[key(x, y)] = m
        }
        for (x in 0 until 100) for (y in 0 until 100) {
            var m = INF
            for (d in -reach..reach) { val yy = y + d; if (yy in 0..99) { val v = rows[key(x, yy)]; if (v < m) m = v } }
            fire[key(x, y)] = m
        }
    }
    ...
    private fun regionSize(start: Int, t0: Int, margin: Int, cap: Int): Int {
        ...
        mark[start] = markGen
        stepDist[start] = t0
        stepQueue[tail++] = start
        while (head < tail && tail < cap) {
            val c = stepQueue[head++]
            val cx = c / 100; val cy = c % 100
            val t = stepDist[c] + 1
            for (dx in -1..1) for (dy in -1..1) {
                if (dx == 0 && dy == 0) continue
                ...
                if (nx < 0 || ny < 0 || nx > 99 || ny > 99) continue
                val n = key(nx, ny)
                if (wall[n] || mark[n] == markGen || fire[n] <= t + margin) continue
                mark[n] = markGen
                stepDist[n] = t
                stepQueue[tail++] = n
            }
        }
        return minOf(tail, cap)
    }

    /** ШАГ К ПРОСТОРУ (v596, см. USE_SCOUT_OPEN_EVADE): из стояния и восьми шагов — тот, после которого область скаута
     *  больше (до SCOUT_OPEN_CELLS: больше — места хватает). При равной — стоять, если на клетке безопасно, иначе клетка,
     *  которую он обстреляет позже. Шаг в клетку, куда его выстрел приходит к следующему тику с запасом меньше
     *  SCOUT_STEP_MARGIN, области не имеет. */
    private fun openStep(s: Creep, ctx: Ctx, here: Int): RefugeMove {
        val occupied = BooleanArray(N)
        for (o in ctx.myCreeps) if (o.id != s.id) occupied[o.key] = true
        for (o in ctx.enemyCreeps) occupied[o.key] = true
        var best = here
        var bestSize = -1
        var bestStay = 0
        var bestFire = -1
        for ((dx, dy) in dirsNow()) {
            ...
            if (x < 0 || y < 0 || x > 99 || y > 99) continue
            val k = key(x, y)
            if (k != here && (wall[k] || occupied[k])) continue
            val size = if (fire[k] > 1 + SCOUT_STEP_MARGIN) regionSize(k, 1, SCOUT_STEP_MARGIN, SCOUT_OPEN_CELLS) else 0
            val stay = if (k == here && size > 0) 1 else 0
            if (size > bestSize || (size == bestSize && (stay > bestStay || (stay == bestStay && fire[k] > bestFire)))) {
                best = k; bestSize = size; bestStay = stay; bestFire = fire[k]
            }
        }
        val why = when {
            bestSize <= 0 -> "cornered"
            best == here -> "stay"
            bestSize >= SCOUT_OPEN_CELLS -> "open"
            else -> "room"
        }
        scoutEvadeWhy.bump(why)
        return RefugeMove(if (best == here) null else InfluenceMap.cell(best / 100, best % 100), why)
    }
    ...
    fun move(s: Creep, ctx: Ctx): RefugeMove? {
        ...
        if (USE_SCOUT_OPEN_EVADE) {
            ...
            if (flag != null) {
                ...
                while (parent[c] != here && parent[c] >= 0) c = parent[c]
                if (parent[c] == here) {
                    scoutEvadeWhy.bump("flag")
                    return RefugeMove(InfluenceMap.cell(c / 100, c % 100), "flag")
                }
            }
            return openStep(s, ctx, here)
        }
        // на своём флаге: держим, пока до обстрела клетки не меньше запаса
        if (ctx.flags.any { it.ours && it.pos.x == s.x && it.pos.y == s.y } && fire[here] >= SCOUT_FLAG_MARGIN) {
            scoutEvadeWhy.bump("hold"); return RefugeMove(null, "hold")
        }
        region(here)
        val blocked = BooleanArray(N)
        for (o in ctx.myCreeps) if (o.id != s.id) blocked[o.key] = true
        ...
        if (flag != null) { target = flag.pos.key; why = "flag" }
            ...
            if (target == here) {
                ...
                for ((dx, dy) in dirsNow()) {
                    ...
                    if (x < 0 || y < 0 || x > 99 || y > 99) continue
                    val k = key(x, y)
                    if (wall[k] || blocked[k]) continue
                    if (fire[k] > bf) { best = k; bf = fire[k] }
                }
                if (best < 0) { scoutEvadeWhy.bump("stay"); return RefugeMove(null, "stay") }
        ...
        var c = target
        while (parent[c] != here && parent[c] >= 0) c = parent[c]
        if (parent[c] != here) { scoutEvadeWhy.bump("stay"); return RefugeMove(null, "stay") }
        scoutEvadeWhy.bump(why)
        return RefugeMove(InfluenceMap.cell(c / 100, c % 100), why)
    }
}
...
internal object Garrisons {
     ...
     *  «дальним из двух расстояний до баз». */
    fun contestedFlag(ctx: Ctx): FlagInfo? = ctx.flags.minByOrNull { f ->
    ...
    /** СНЯТЬ ЛАГЕРЬ (v605, см. USE_CAMP_BREAK). Этапы `Memory.campBreak[0]`: 0 — прежнее поведение, пока его CAMP_MIN боевых
     *  не простоят у оспариваемого флага CAMP_TICKS тиков подряд; тогда армия делится на отряды по GARRISON_SIZE, и 1 — все идут
     *  на оспариваемый флаг (натиск всей армией); 2 — первый отряд на флаге, остальные стоят рядом GARRISON_SETTLE тиков, пока он
     *  устаивается; 3 — остальные идут на второй флаг (самый дорогой, из равных — ближний к нашей армии); 4 — второй отряд на
     *  нём, третий стоит рядом GARRISON_SETTLE; 5 — третий идёт на свой флаг (ближний ко второму, из равных — дороже).
     *  Погиб отряд, которого ждёт этап, — этап пропускается. */
    fun campBreak(ctx: Ctx) {
        ...
        if (s[0] == 0) {
            ...
            for ((si, sq) in squadsOf(fighters, n).withIndex()) for (c in sq) {
                ...
                Memory.garrisonFlag[c.id] = targets[0]
            }
        ...
        fun alive(si: Int) = ctx.myCreeps.any { Memory.garrisonSquad[it.id] == si }
        fun on(si: Int, fk: Int) = ctx.myCreeps.any { Memory.garrisonSquad[it.id] == si && it.key == fk }
        fun retarget(from: Int, fk: Int) { for (c in ctx.myCreeps) if ((Memory.garrisonSquad[c.id] ?: -1) >= from) Memory.garrisonFlag[c.id] = fk }
    ...
    fun assign(ctx: Ctx) {
        ...
        for ((si, sq) in squadsOf(fighters, flags.size).withIndex()) for (c in sq) Memory.garrisonFlag[c.id] = flags[si].pos.key
    }
    ...
    fun step(creep: Creep, ctx: Ctx): Pair<Position?, Boolean> {
        if (!active(creep.id)) return Pair(null, false)
        ...
        if (here in cells) {
            if (fk !in taken) {
                // на клетку флага — из того отряда, чей это флаг навсегда (v605: на время натиска у флага стоит вся армия)
                val first = ctx.myCreeps.filter { (Memory.garrisonHome[it.id] ?: Memory.garrisonFlag[it.id]) == fk && it.key in cells }
                    .minByOrNull { it.id }
        ...
        // клетка флага — только тому отряду, чей это флаг навсегда (v605, стенд: крип другого отряда встал на неё первым)
        val homeHere = (Memory.garrisonHome[creep.id] ?: fk) == fk
        val free = cells.filter { it !in taken && (homeHere || it != fk) }.minByOrNull { maxOf(abs(it / 100 - creep.x), abs(it % 100 - creep.y)) }
...
/** Прибор уклонения скаута (v576): `scev=` — ветки хода (flag / hold / home / hide / cornered / stay; v596: open / room). */
...
internal class MeasuresForces(private val ctx: Ctx) {
    ...
    init { kiteMassed = enemyMassedNow }
}
...
internal class MeasuresExchange(private val ctx: Ctx, private val forces: MeasuresForces) {
    ...
    val now = getTicks()
    val enemyHitsNow = forces.enemyCreeps.sumOf { it.hits }
    init { if (lastEnemyHitsTotal >= 0 && enemyHitsNow < lastEnemyHitsTotal) enemyDamageTaken += lastEnemyHitsTotal - enemyHitsNow }
    init { lastEnemyHitsTotal = enemyHitsNow }
    val ourHitsNow = ctx.army.sumOf { it.hits }
    init { Memory.enemyHitsHist.addLast(enemyHitsNow); Memory.ourHitsHist.addLast(ourHitsNow) }
    init { while (Memory.enemyHitsHist.size > STALL_TICKS) Memory.enemyHitsHist.removeFirst() }
    init { while (Memory.ourHitsHist.size > STALL_TICKS) Memory.ourHitsHist.removeFirst() }
    ...
    init { Memory.ledgerHist.addLast(exchangeLedger) }
    init { Memory.ourLostHist.addLast(ourDamageTaken) }
    init { Memory.hisLostHist.addLast(enemyDamageTaken) }
...
internal class MeasuresChase(private val ctx: Ctx, private val forces: MeasuresForces, private val exchange: MeasuresExchange) {
    ...
    val huntable = forces.armedEnemies.filter { catchable(it, chasers) }
    init { cpuMark("a.hunt") }
    ...
    val meleeAdjacent = forces.combatEnemies.any { e -> hasMelee(e) && ctx.army.any { getRange(e, it) <= 1 } }
    ...
    private val nearArmed = forces.armedEnemies.count { e -> forces.strikers.any { getRange(it, e) <= ENGAGE_RANGE } }
    private val nearCatchable = huntable.count { e -> forces.strikers.any { getRange(it, e) <= ENGAGE_RANGE } }
    ...
    // ...и ДО ПЕРВОГО БОЯ пикет — не край ВСЕЙ его армии (v642, см. USE_PICKET_GROUP): связная группа его боевых вокруг
    ...
    private val marchCell = centroidOf(ctx.armedArmy.ifEmpty { ctx.army })?.let { it.key } ?: -1
    init { Memory.marchHist.addLast(marchCell) }
    ...
    private val ourArmedCentroid = centroidOf(ctx.armedArmy.ifEmpty { ctx.army })
    ...
    private val pausedChase =  Memory.prevPosture == Posture.HOLD &&
        forces.armedEnemies.any { e -> ctx.army.any { getRange(e, it) <= pauseReach } }
    init {
        if ((Memory.prevPosture == Posture.ANNIHILATE || pausedChase) && !fightOn && armyDist >= 0) {
            ...
            Memory.enemyCentHist.addLast((centroidOf(forces.armedEnemies))?.let { it.key } ?: -1)
        } else { Memory.armyDistHist.clear(); Memory.enemyCentHist.clear() }
    }
    init { while (Memory.armyDistHist.size > DETACH_WINDOW + 1) Memory.armyDistHist.removeFirst() }
    init { while (Memory.enemyCentHist.size > DETACH_WINDOW + 1) Memory.enemyCentHist.removeFirst() }
    ...
    private fun kept(window: Int, needMoved: Boolean = true): Boolean {
    ...
    private val distanceKept = forces.combatEnemies.isNotEmpty() && kept(DETACH_WINDOW)
    init { if (distanceKept) lastDistanceKeptTick = exchange.now }
    ...
    private val corneredInReach =  forces.armedEnemies.size >= SPLIT_MIN && run {
        val still = forces.armedEnemies.filter { e ->
    ...
    init { corneredWas = cornered }
    private val keepsDistance = !cornered && (
    ...
    init {
        if (!netDamage && exchange.now >= stallUntil &&
            ((forces.combatEnemies.isNotEmpty() && preyNearTicks >= STALL_TICKS) || marchStalled || keepsDistance)) {
            stallUntil = exchange.now + STALL_COOLDOWN
            if (DEBUG_LOG) println("stall t=${exchange.now}: ${if (marchStalled) "the march has not moved a cell for $MARCH_STALL_TICKS ticks" else if (keepsDistance) "the enemy keeps its distance (${Memory.armyDistHist.first()} -> ${Memory.armyDistHist.last()} over $CHASE_WINDOW ticks)" else "picket of $nearArmed armed in reach for $preyNearTicks ticks"} without damage either way — flags until $stallUntil")
        }
    }
...
internal class MeasuresFight(private val ctx: Ctx, private val forces: MeasuresForces, private val exchange: MeasuresExchange, private val chase: MeasuresChase) {
    ...
    init {
        if (leadingNow && forces.armedEnemies.isNotEmpty()) survLead.n++
        Memory.theirsHist.addLast(theirs)
    }
    init { while (Memory.theirsHist.size > MEASURE_WINDOW) Memory.theirsHist.removeFirst() }
    ...
    private val nearRange = if (Memory.prevPosture == Posture.RETREAT) NEAR_RANGE + NEAR_RELEASE else NEAR_RANGE
    ...
    val enemyNear = forces.armedEnemies.any { e -> ctx.army.any { getRange(e, it) <= nearRange } }
    ...
    val contact = inContact(forces.armedEnemies, massArmy)
    ...
    private val outmatchedNow =  contact && forces.armedEnemies.isNotEmpty() &&
        ourDamageTaken > 0 && enemyDamageTaken < ourDamageTaken * BREAK_OFF_RATIO
    init { outmatchedTicks = if (outmatchedNow) outmatchedTicks + 1 else 0 }
    ...
    private val theirPeriod = forces.combatEnemies.filter { canMove(it) }.minOfOrNull { plainPeriod(it) } ?: Int.MAX_VALUE / 4
    ...
    val retreatFeasible = (!contact || forces.strikers.isEmpty()) && !atRetreatPoint
    init { cpuMark("a.retreat") }
}
...
internal class RememberTick(private val ctx: Ctx, private val meas: ArmyMeasures?, private val strat: ArmyStrategy?, private val stance: ArmyStance?) {
    ...
    init {
        for (e in ctx.enemyCreeps) {
            val cell = e.key
            if (Memory.enemyPrevCell[e.id] != cell) Memory.enemyLastMove[e.id] = getTicks()
        }
    }
    init { Memory.enemyLastMove.keys.retainAll { id -> ctx.enemyCreeps.any { it.id == id } } }
    init { Memory.enemyPrevCell.clear() }
    init { for (e in ctx.enemyCreeps) Memory.enemyPrevCell[e.id] = e.key }
    // история движения — для ловимости (см. evasive)
    val armedCentroid = centroidOf(ctx.armedArmy.ifEmpty { ctx.army }) ?: ctx.ourCentroid
    init { Memory.ourCentroidHist.addLast(armedCentroid.key) }
    init { while (Memory.ourCentroidHist.size > CHASE_WINDOW) Memory.ourCentroidHist.removeFirst() }
    init {
        for (e in ctx.enemyCreeps) {
...
internal fun readSignals(ctx: Ctx) {
    ...
    // v679: центр его вооружённых на НАШЕЙ половине — он пришёл к нам (см. USE_RANGED_FRONT_BEFORE_CLASH)
    Signals.hisFistInOurHalf = centroidOf(armedNow)?.let { c -> getRange(c, ctx.home) < getRange(c, ctx.enemyHome) } == true
    ...
    if (Signals.fightImminentNow && fightImminentTicks == 0)
        rushStartDist = ctx.enemyCentroid?.let { getRange(ctx.ourCentroid, it) } ?: 0
    fightImminentTicks = if (Signals.fightImminentNow) fightImminentTicks + 1 else 0
    ...
    Signals.enemyNotFightingNow =  noFireTicks >= STALL_TICKS && !enemyWithinReach
...
/** СИГНАЛЫ ТИКА (v459, второй шаг архитектуры, этап 6): то, что `readSignals` говорит стадиям ниже, — как есть. Объект, а не носитель: `unflaggedRushNow` и `groupSafe` — защёлки, читающие собственное значение прошлого тика, а `readSignals` стоит после зонда первых тиков и перед бегунами — место в тике несущее. */
internal object Signals {
    ...
    internal var hisFistInOurHalf = false                  // v679: центр его вооружённых ближе к нашему дому, чем к своему
