 ...
 *  5. `cmdDetach` командир очищает и строит заново в тик гонки, и очистка стоит ЗА ранними выходами «бой без держателей» и
 *     «свободных нет» (v465, дефект 4): на них прежний состав переживает тик; выходы «бой при держателях» и «бюджет исчерпан»
 *     оставляют одних держателей, как прежде. Гарнизон (`garrisonOf.retainAll`) по-прежнему читает состав ТЕКУЩЕГО построения.
 ...
 * 10. Мёртвых из `cmdDetach` снимает [pruneDead] в `Ctx` (v465, дефект 4); в режиме боя без `fightOnNow` и при пустой армии набор не
 *     трогает никто — это считает `cmdinert=`.
...
internal object Squads {
    ...
    /** Снять из набора командира погибших (v465, дефект 4): до того набор чистили только [recallAll] и отзыв боем, и id погибших
     *  лежали в нём до следующей гонки. Зовёт мир (`Ctx`), рядом с [keepDetached]. */
    fun pruneDead(living: List<Creep>) { cmdDetach.retainAll { id -> living.any { it.id == id } } }
