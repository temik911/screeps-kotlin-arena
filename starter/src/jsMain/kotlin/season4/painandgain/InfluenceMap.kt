...
object InfluenceMap {
    ...
    class HealNeed {
        ...
        fun stampHealNeed(allies: List<Creep>) {
            ...
            for (a in allies) {
                ...
                // ...И РАЗДЕТЫЙ БОЕЦ — НЕ ОБУЗА, А НАИБОЛЬШИЙ ВОЗВРАТ (v517, см. USE_NEED_DISARMED_FIGHTER). Скидка
                // NEED_DISARMED пришла общим коммитом полей 10.09.2026 и ни разу не судилась: ни абзаца версии, ни
                // записи в архиве вердиктов. Между тем арифметика класса говорит обратное — мили вплотную даёт 240 в
                // тик против 60 у стрелка и 72 у лекаря, то есть вернуть оружие раздетому мили ценнее любой другой
                // доставки в армии; тело `a8m8` несёт восемь ATTACK впереди (800 из 1600), и первая ударная часть
                // возвращается, только когда его вылечат ВЫШЕ 800.
                // Замер восьми реплеев (21.09.2026): падение ниже 800 случается одинаково в победах и поражениях, и
                // во всех восьми первым до нуля доходит ОН (на 3-67 тиков раньше нас), а различает исходы ВОЗВРАТ —
                // навсегда без оружия остаются 0/2/0/0 наших мили из четырёх в победах против 4/4/4/4 в поражениях,
                // разоружены они 0-10 % времени против 29-42 %, и в 97-100 % тиков разоружённого рядом стоит лекарь
                // против 21-37 % тиков, где рядом НЕТ НИ ОДНОГО. Скидка бьёт ровно по этому.
                // Скидка остаётся для того, кому возвращать нечего: крип, рождённый без оружия (лекарь, скаут)
                val bornArmed = a.body.any { it.type == ATTACK || it.type == RANGED_ATTACK }
                val disarmedMul = if (USE_NEED_DISARMED_FIGHTER && bornArmed) 1.0 else NEED_DISARMED
                val need = base * (if (armed) 1.0 else disarmedMul)
                needLeft[a.id] = need
                total += need
    ...
    var published: HealNeed? = null
        set(v) { field = v; publishedWrites++ }

    /** Сколько раз поле нужды уходило в мир — владелец считает свои записи (v460, прибор `impure=`, см. armyCommand): во время
     *  перебора замыслов командира это число меняться не должно. */
    var publishedWrites = 0
