...
object PainAndGain {
    ...
    private const val USE_COMMANDER_EVERY_FIGHT = false
    private const val COMMAND_MIN_FOES = 3
    // перепроверено на честном приборе (v157, гейт с врагом, который бьёт по лекарям): по-прежнему 132 из 135 —
    // camp 2 970:21 279, scatter и kite. Дело не в том, что прибор чего-то не видел: раздача клеток не умеет вести
    // бой против того, кто строем не дерётся, и порог по числу его вооружённых этого не меняет
