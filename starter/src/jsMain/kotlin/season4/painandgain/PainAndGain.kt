...
object PainAndGain {
    ...
    private const val BOT_VERSION = "v236"
...
cpuMark("arrival")
    ...
    private fun captureCost(ctx: Ctx, f: FlagInfo): Double {
        ...
        val side = if (USE_CAPTURE_COST_ONE_SIDE )
...
cpuMark("r.cands")
    ...
    private fun postPoint(ctx: Ctx): Position {
        ...
        val ourHalfFlag = if (USE_POST_OUR_FLAG) ctx.flags.filter { DistanceMap.inOurHalf(it.pos.x, it.pos.y) }
            .minByOrNull { getRange(it.pos, ctx.ourCentroid) }?.pos else null
...
cpuMark("a.retreat")
        ...
        if (USE_DETACH) {
            ...
            if (USE_ONE_CORE_MEASURE && farmer && detachedIds.isNotEmpty()) {
                ...
                while ( detachedIds.isNotEmpty() && core.any { hasWeapon(it) } && ourPowerOf(core, recallRef) < enemyPowerOf(recallRef, core) * theirsDown * floorNow) {
...
cpuMark("a.evade")
        ...
        val focusTarget = if (USE_FOCUS_STICKY && focusPrev != null && !killableNow && !rangedNow && !moreGuns && InfluenceMap.profileOf(focusPrev).let { it.melee + it.ranged + it.heal > 0.0 } &&
        ...
        if (commanderNow) {
            ...
            if (!USE_SIMULATION || cpuTight) commandFight(commandArmy, combatEnemies, armedEnemies, commandOf, Intent.PRESS, ourFlagCells = ourFlagCells)
            else {
        ...
        for (creep in army) {
            ...
            val rotating = USE_ROTATION && !healer && hasWeapon(creep) && healersAlive && run {
                ...
                if (creep.id in rotatingIds) { if (backIn) { rotatingIds.remove(creep.id); if (DEBUG_LOG) println("rot t=$now in ${creep.id} frac=$frac took=${now - (rotateSince[creep.id] ?: now)}"); false } else true }
                    ...
                    rotatingIds.add(creep.id); rotateSince[creep.id] = now; rotOut++; if (DEBUG_LOG) println("rot t=$now out ${creep.id} frac=$frac hits=${creep.hits}"); true
                } else false
            }
            ...
            val inLine = !USE_INLINE || spotNow || guardNow || (formationReady && combatArmy.count { it.id != creep.id && hasWeapon(it) && getRange(creep, it) <= FORM_RANGE } >= 2)
