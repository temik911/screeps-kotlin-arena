...
internal fun PainAndGain.armyStrategy(ctx: Ctx, seg: ArmyStrategyIn): ArmyStrategyOut = with(seg) {
    ...
    val decision = Strategist.decide(Strategist.Inputs(
        ...
        // ...И ТОЛЬКО ПОКА ЕГО МИЛИ НЕ ВПЛОТНУЮ (v384). Отказ от строя требует `underFire`, то есть срабатывает уже В
        // КОНТАКТЕ, — а уйти из контакта нельзя: по реплеям после нашего шага прочь его вооружённый остаётся в трёх
        // клетках через 6 тиков в 64 % случаев и через 10 в 51 %, и ни один из погибших не оторвался ни разу.
        // Замер v383 против сильнейших кулаков это и показал: против `MetalicaX#9` строй берётся 4 % тиков, убито им
        // 0,0 его крипов и потеряно 7,38 наших тел — при том что флагов мы держим 2,17 против его 1,13 и по очкам
        // ведём +7 220. Мы отказываемся драться там, где выбора уже нет, и нас выбивают порознь. Пока его мили в
        // одной клетке от наших — строй берётся всегда
        enemyMassed = enemyMassedSignal && !theirMeleeIn &&
