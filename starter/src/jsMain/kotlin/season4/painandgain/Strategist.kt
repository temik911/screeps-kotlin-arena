...
internal fun PainAndGain.armyCommand(ctx: Ctx, seg: ArmyCommandIn): ArmyCommandOut = with(seg) {
    ...
    if (commanderNow) {
        ...
        if (cpuTight) commandFight(commandArmy, combatEnemies, armedEnemies, commandOf, Intent.PRESS, ourFlagCells = ourFlagCells)
            ...
            // (руки v250 и v253, серия v245). Теперь первый замысел оценивается всегда, а следующий — только если время тика
            // плюс цена прошлой пробы плюс запас на хвост тика укладываются в лимит движка. Все три числа вычисляются: лимит —
            // arenaInfo.cpuTimeLimit, цена пробы — замер прошлой, запас — наибольший замеренный хвост после командира
            // (cmdTailMax, считается в конце tickBody). На стенде тики в разы дешевле лимита — перебор не режется никогда
            val limitMs = arenaInfo.cpuTimeLimit / 1_000_000.0
            var lastCost = 0.0
            srchTicks++
            for (intent in Intent.values()) {
                if (bestPlan != null && cpuMs() + lastCost + cmdTailMax > limitMs) { srchCut++; break }
                val t0 = cpuMs()
