...
internal fun PainAndGain.captureBlock(ctx: Ctx, f: FlagInfo): String? {
    ...
    val foughtFoe = USE_GATE_VS_FIGHTER && fightMassedSeen && !ctx.passiveEnemy
    val opp = if (foughtFoe) ctx.combatEnemies else oppLocal
    capOppSum += opp.size
    ...
    if (groupSafe && !foughtFoe) return null
