...
internal fun PainAndGain.captureBlock(ctx: Ctx, f: FlagInfo): String? {
    ...
    if (!losingRace && !stalledNow && !intercept && contactArmy.any { fullSpeed(it) && hasWeapon(it) } && inContact(foes, contactArmy)) {
        ...
        // ...И ВЕТО ПЛАТИТ ТОЛЬКО ТОТ КОНТАКТ, В КОТОРОМ ИДЁТ РАЗМЕН (v431). Прибор `warmcap` заведён в v221 ровно
        // для этого вопроса — «сколько отказов выдано контактом, где за окно ни одна сторона не потеряла
        // STALL_DAMAGE», — и три сессии оставался ТОЛЬКО прибором, хотя ответ у него однозначный: против
        // `Coldkimchi#1` это **1 096 отказов из 4 636, то есть 24 %**. Дебафф флага глобален, но платится он
        // БОЕМ: пока хиты не тают, лишние 20 % к его урону не стоят ничего, а очки идут. Против этого соперника
        // цена пропуска видна прямо в форме матчей — все сборки оканчивают бой с ЦЕЛОЙ армией (99–100 % хитов) и
        // проигрывают по очкам 2,69 против 2,81 и 2,97 против 3,04 флага. Третий случай одной болезни в этом боте
        // («правило измеряется, но не действует»): так же были не подключены страховка нулевой мощи v227 и правило
        // лекаря за линией v200, жившее внутри отключённого строя
        // ...и снимается вето ТОЛЬКО ПРОТИВ СОМКНУТОГО врага. Первая редакция снимала его при любом контакте без
        // размена, и гейт назвал цену: `match34:scatter` из победы превратился в поражение по очкам 20 375 : 24 325.
        // Причина по существу: у РОССЫПИ контакт означает стычки у флагов, где размен то вспыхивает, то гаснет, и
        // «сейчас никто не теряет хитов» там не значит «боя не будет». У сомкнутого блоба контакт без размена — это
        // именно стояние друг против друга, за которое дебафф не платится
        if (!USE_CAP_ONLY_WHEN_TRADING || exchangeLiveNow || !enemyMassedSignal) return capCount(f, "contact.mass")
    }
    ...
    if (firstFightAhead && !behindOnScore && !stalledNow && !intercept) {
        ...
        if (ourAfter > hisAfter) return capCount(f, "first.fight")
    }
    ...
    if (raceWon && enemyMassedSignal && !ctx.passiveEnemy && !stalledNow && !intercept) return capCount(f, "enough")
...
internal fun PainAndGain.commandRace(ctx: Ctx, army: List<Creep>, armedEnemies: List<Creep>, flags: List<FlagInfo>,
    ...
    val fightBlocks = if (groupSafe) coreContactNow else fightOnNow
