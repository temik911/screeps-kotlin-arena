...
internal fun PainAndGain.captureBlock(ctx: Ctx, f: FlagInfo): String? {
    ...
    val firstFightAhead = firstFightTick == 0 && enemyMassedSignal && !ctx.passiveEnemy
...
internal fun PainAndGain.updateKeepers(ctx: Ctx, army: List<Creep>) {
    ...
    // ...А СНИМАЕТ ЕГО ДАВЛЕНИЕ (v307, см. PRESS_COUNT): сравнение сил (v305) сгоняло нашего мили с флага одним его
    // стрелком — 34–43 снятия за матч, — хотя флаг стоит очков, а вернуть его стоит дюжины тиков
    for (f in ctx.flags) {
        val press = ctx.combatEnemies.count { getRange(f.pos, it) <= PRESS_CELLS }
        Memory.flagPress[f.id] = if (press >= PRESS_COUNT) (Memory.flagPress[f.id] ?: 0) + 1 else 0
    }
    fun pressedAt(f: FlagInfo): Boolean = (Memory.flagPress[f.id] ?: 0) >= PRESS_TICKS
    ...
    while (iter.hasNext()) {
        ...
        val stay = onFlag && (if (groupSafe) coreHolds(core) && !pressedAt(f!!)
...
internal fun PainAndGain.commandRace(ctx: Ctx, army: List<Creep>, armedEnemies: List<Creep>, flags: List<FlagInfo>,
    ...
    // ⚠️ ОТВЕРГНУТО (v301): «в режиме пар бой вне контакта ядра выпуска не останавливает». Против けろびー оно не нужно —
    // без него командир и так раздавал 3–5 тыс. назначений за матч, — а на стенде оно отправляло пару к флагу посреди
    // перестрелки и уносило из ядра двоих: фермеры стенда сидят на своих флагах, их берёт только сила ядра, и
    // match33:scatter упал (21 199:24 238), match28:scatter 17 277 -> 23 979 и match34:scatter 21 776 -> 23 812 у него
