...
internal fun PainAndGain.captureBlock(ctx: Ctx, f: FlagInfo): String? {
    ...
    // ...И ПРОТИВ ТОГО, КТО НЕ БЬЁТ НАШИХ В ГРУППЕ, ПАРИТЕТ НЕ СТОРОЖ (v328, см. GROUP_SAFE_DMG). Доктрина паритета
    // бережёт армию от дебаффа перед боем, которого здесь не будет: он от наших групп отходит. А цена запрета выросла
    // ровно тогда, когда тела встали на флаги: мощь ядра падает вместе с дебаффами, и ворота отказывают по «паритету»
    // 1 382 раза за матч (v327) — захватов стало 17 против 37, флагов 1,41 против 1,89
