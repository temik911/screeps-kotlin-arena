...
/** ВОРОТА ЗАХВАТА ПРОТИВ СОМКНУТОЙ НЕПАССИВНОЙ АРМИИ СЧИТАЮТ ЕЁ ВСЮ (v433, см. captureBlock). Дебафф флага глобален и
 *  не снимается до конца матча, поэтому его платит бой, который СЛУЧИТСЯ, а не тот, которого сейчас нет. Два обхода
 *  пола паритета в тех же 13 играх: локальный противник вне контакта пуст (0,10–0,30 его крипа на флаг при 10–11 боевых
 *  на карте) — powerOf(пусто) даёт ноль и пол вырождается в «ours >= 0» (4 захвата из 21); и groupSafe, писанный под
 *  фермера, который не бьёт вовсе, снимал пол в паузах между боями (8 захватов из 21, в 6aae34db 750 тиков подряд, при
 *  879 против пола 2 672). Различитель — уже существующий «сомкнут и не пассивен», тот же, что у ворот первого боя. */
internal const val USE_GATE_VS_MASSED = true
