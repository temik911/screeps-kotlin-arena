...
/** Лекарь становится за своего вооружённого и вне строя (v418, см. placeScored): правило v200 меряется прибором
 *  `hline=`, но живёт внутри `planBlock`, а против блоба строй отключён — в каждом разгроме `hline=0/0`. Разбор
 *  шести матчей против MetalicaX: его стволов в трёх клетках от нашего лекаря 3,94 ± 0,47 в поражениях против 1,69
 *  в победе (z = −4,8), его части лечения на контакт+40 целы 17–18 из 18 в поражениях и 0 из 18 в победе.
 *  Требование мягкое: нет отвечающей клетки — лекарь ставится как прежде, это считает `hbl=`. */
internal const val USE_HEAL_BEHIND_LINE = true
