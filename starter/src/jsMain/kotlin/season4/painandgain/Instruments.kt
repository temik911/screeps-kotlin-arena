...
internal fun PainAndGain.printTick(ctx: Ctx, seg: PrintTickIn): PrintTickOut = with(seg) {
    ...
    if (DEBUG_LOG && getTicks() % LOG_EVERY == 0) {
        ...
        println(
            ...
            " poised=$poisedTicks/$poisedAll edge=$edgeSpot/$edgeAll capopp=$capOppSum/$capAllSum ffight=$firstFightTick stray=$strayCapRefused sout=$soutOut/$soutBack/$soutTicks hold=$holdPinned/$holdArmedStay/$holdKeptRace/$holdKeptFight gsafe=$groupSafeTicks/$groupDmgWindow fguard=$flagGuardTicks route=$routeKept man=$manned gcov=$garCovered/$garAll courier=$courierTicks hunt=$huntTicks/$huntCreepTicks toothless=$pushToothless sit=$flagSitOcc/$flagSitAll keep2=$keepOn/$keepTicks/$keepOffCore:$keepOffPack:$keepOffLeft keep3=$keepOffGone:$keepOffFlag:$keepOffMoved:$keepOffHurt" +
...
internal const val BOT_VERSION = "v398"
