...
internal fun PainAndGain.runRunners(ctx: Ctx) {
    ...
    for (s in runners) {
        ...
        // ...а КРИП ПОСТОЯННОГО ГАРНИЗОНА УХОДИТ ОТ ДВОИХ, А НЕ ОТ ОДНОГО (v339, см. GARRISON_FLAGS). «Не уходит вовсе»
        // (v338) замерено хуже: наших флагов 1,82 против 2,60 у v337, тел на флагах 1,28 против 2,02 — стоящий до конца
        // гибнет и выбывает насовсем, а ушедший возвращается. Один его стрелок флага не стоит: пока он один, держатель
        // с лекарем (v334) переживает размен
        val garrisonStays = groupSafe && Memory.garrisonOf[s.id] != null && holds.containsKey(s.id) &&
            s.hits * 2 >= s.hitsMax &&
            ctx.combatEnemies.count { getRange(s, it) <= SCOUT_FLEE_TRIGGER && threatening(it, ctx.enemyCreeps) } < 2
