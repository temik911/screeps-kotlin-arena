...
internal fun PainAndGain.runRunners(ctx: Ctx) {
    ...
    for (s in runners) {
        ...
        // ...а КРИП ПОСТОЯННОГО ГАРНИЗОНА НА СВОЁМ ФЛАГЕ НЕ БЕЖИТ ОТ БЛИЗОСТИ СТВОЛОВ (v338, см. GARRISON_FLAGS): к нему
        // приставлен лекарь (v334), а уход стоит флага — его крип входит на пустую клетку через 8 тиков после нашего
        // ухода. Уходит он только когда хитов меньше половины. Прежняя редакция (v327) распространяла это на ВСЕХ
        // держателей и роняла темп захватов вдвое; здесь правило держит только четвёрку закреплённых
