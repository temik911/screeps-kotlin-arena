...
internal class RunnerMatch(private val ctx: Ctx, private val runners: List<Creep>) {
    ...
    init {
        if (!cpuGuard) for (s in runners) {
            ...
            if ((USE_SCOUT_MARGIN_EVADE && tourerMode()) && !bornCombatant(s)) continue   // скаут сам выбирает флаг по запасу времени (v576)
            ...
            for (f in ctx.flags) {
                ...
                val value = gain * horizon *
                    (if (f.id == currentId) 1.25 else 1.0) * (if (!f.ours && !captureAllowed(ctx, f, Prev.exchange, CapAsker.RUNNER, serious = false)) 0.2 else 1.0)   // оценка, не ворота (v451, capeval=)
    ...
    init {
        for (s in runners) {
            ...
            if (canMove(s) && !bornCombatant(s)) {
                val mv = if ((USE_SCOUT_MARGIN_EVADE && tourerMode())) ScoutEvade.move(s, ctx) else refugeMove(s, ctx)
                if (mv != null) {
            ...
            val block = captureBlock(ctx, f, Prev.exchange, CapAsker.RUNNER)
