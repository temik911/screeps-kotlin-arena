...
internal object Forecast {
    ...
    /** ПОЗИЦИИ ВРАГА НА t+1 (v246, план: Forecast.predict): та же модель шага, что в прокате [simulate] — мили к ближайшему
    ...
    /**
     * АДРЕСНАЯ ОПАСНОСТЬ t+1 (v246, план: Forecast.threatAt) — ДОБАВОЧНЫЙ терм к полю E в оценке клетки бойца, а не замена
     * ему (замена в v244 отвергнута живым A/B 0-8/0-8: клетка, где крип «не цель», читалась как безопасная при десяти
     * стволах над ней). Терм — урон тех его стволов, которые, шагнув по модели ([predictCells]), выберут именно крипа c в
     * клетке p: лекарь в досягаемости первым, иначе ближайший, при равенстве меньшие хиты — та же модель, что у
     * [wallTargetOf] и у прибора adr=. Остальные наши стоят на НАЗНАЧЕННЫХ клетках (план сравнивается с планом, как у
     * согласованности строя). Досягаемости — после его шага: стрелок RANGED_RANGE, мили 1. Лучший «другой» кандидат каждого
     * ствола считается один раз на крипа ([prepare]) и пересчитывается, когда раздача перешла к другому, — терм стоит
     * O(стволов) на клетку, а не O(стволов × наших), как замена в v244 (+13 мс к plan= при потолке 100).
     */
    internal class Threat(his: List<Creep>, pred: Map<String, Position>) {
        ...
        fun prepare(c: Creep, ours: List<Creep>, cellOf: (Creep) -> Position) {
            ...
            for (f in ours) {
                ...
                val p = cellOf(f)
                val h = PainAndGain.hasHeal(f) && !PainAndGain.hasWeapon(f)
        ...
        /** Урон по крипу c в клетке p от стволов, для которых он — цель; [prepare] уже вызван для c. */
        fun at(c: Creep, p: Position): Double {
            if (guns.isEmpty()) return 0.0
            val h = PainAndGain.hasHeal(c) && !PainAndGain.hasWeapon(c)
            var sum = 0.0
            for (i in guns.indices) {
                val g = guns[i]
                val d = maxOf(abs(p.x - g.x), abs(p.y - g.y))
            ...
            return sum * InfluenceMap.takenOf(c)
        }
    }
}
