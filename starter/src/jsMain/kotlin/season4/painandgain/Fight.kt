...
internal fun PainAndGain.shoot(creep: Creep, enemyCreeps: List<Creep>, focusTarget: Creep?, focusOrder: List<Creep>) {
    ...
    // ⚠️ ...И ЭТО ПРАВИЛО НАКОНЕЦ ПРИМЕНЕНО (v389, дефект v218). Абзац выше описывает его с v218, переменная
    // `focusBreakableNow` исправно считается в Tactician (killTicks фокуса конечен, то есть дотягивающиеся стволы
    // пробивают его лечение), — но в само условие она не входила, и веер брался всегда, едва рядом оказывались три
    // его крипа. Цена измерена против `MetalicaX#9`: 201 веерный выстрел из 972 (21 %, у других кулаков 0,3–1 %),
    // убито за восемь матчей 0,0 его крипов, и ни один за пять реплеев не опускался ниже 53 % хитов. Ровно тот
    // случай, который абзац и описывает: веер разводит сошедшиеся стволы и отдаёт цель его лекарям
    if (massValue > (if (enemyHeals) 2.5 else 1.0) && !focusBreakableNow) {
