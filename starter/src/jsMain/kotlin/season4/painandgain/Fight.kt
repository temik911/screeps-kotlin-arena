...
internal fun PainAndGain.healAndShoot(active: List<Creep>, allies: List<Creep>, enemyCreeps: List<Creep>, focusTarget: Creep?, focusOrder: List<Creep>) {
    ...
    // ...И РОЛЬ ВЕСИТ (v377). Разбор 5 реплеев против топ-3 `●ω<♥♪#6`: в поражениях наше лечение уходит в мили на
    // 52–54 %, В САМИХ ЛЕКАРЕЙ на 34 % и в стрелков лишь на 13–17 %; в победах — 60–61 / 10–13 / 25–31, у соперника
    // 58–67 / 0–4 / 32–40. При этом первыми гибнут именно наши стрелки (первые 3–4 смерти во всех трёх поражениях), а
    // стрелок ниже половины хитов остаётся без лекаря в трёх клетках по 13 и 25 крипо-тиков. Цена роли считается
    // прямо: живой стрелок даёт 60 урона в тик, пятеро — 300 против его потолка лечения 216 (три лекаря вплотную),
    // то есть стрелки и есть то, чем его лечение пробивается; лекарь же лечит себя сам
    fun roleWeight(target: Creep): Double = when {
        hasHeal(target) && !hasWeapon(target) -> HEAL_RANK_MEDIC
        hasRanged(target) -> HEAL_RANK_RANGED
        else -> 1.0
    }
    fun rank(target: Creep): Int = (need(target) * roleWeight(target)).toInt()
    for (creep in active) {
...
/** Вес роли при выборе, кого лечить (v377, см. rank): лекарь себя лечит сам, стрелок — то, чем пробивается его
 *  лечение. Основание: 34 % нашего лечения уходит в лекарей при 13–17 % в стрелков, тогда как первыми гибнут
 *  стрелки; у соперника в лекарей уходит 0–4 %. */
internal const val HEAL_RANK_MEDIC = 0.7
internal const val HEAL_RANK_RANGED = 1.15
