...
internal class DealRecord {
    ...
    /** ЦЕНА ВЫСТРЕЛА ПО ЧИСТОМУ ВКЛАДУ (v482, `shotnet=`, см. USE_RANGED_SHOT_NET): назначений стрелку, где клетка даёт
     *  выстрел, ЧТО-ТО добавляющий к чистому урону армии (цель пробивается с ним) / где достижимая вооружённая цель есть,
     *  а вклад нулевой — его лечение перекрывает и с этим выстрелом / всего назначений стрелку. Вторая часть и есть
     *  предмет: выстрел, за который расстановка платит и которого в размене не будет. */
    val shotPierce = g.counter("shotnet"); val shotWasted = g.counter("shotnet", 1); val shotAll = g.counter("shotnet", 2)
    /** ЛЕЧЕНИЕ ДОХОДИТ ЛИ ДО СТРЕЛКОВ (v494, `rhadj=вплотную к теряющему хиты стрелку/такой стрелок был в шаге от
     *  досягаемости/назначений лекарям при раненом стрелке`). `hadj=` считает прилегание к любому раненому, а в
     *  контакте раненый мили есть почти всегда — доля получается высокой и о стрелках не говорит ничего. Между тем
     *  `rloss=` живьём даёт 62-99 % потерь стрелков от его залпа, то есть спасает их только лечение. */
    val rhadjN = g.counter("rhadj"); val rhadjNear = g.counter("rhadj", 1); val rhadjAll = g.counter("rhadj", 2)
    /** СТРЕЛКИ ПЕРВЫМ РЯДОМ (v680, `rfirst=`, см. USE_RANGED_FRONT_BEFORE_CLASH): тиков, где выбранная раздача ставила
     *  стрелков раньше мили и держала мили за их рядом (`Deal.rangedFirst`). До v680 сработку правила было не увидеть. */
    val rangedFirstN = g.counter("rfirst")
}
