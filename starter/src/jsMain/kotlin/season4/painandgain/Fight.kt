...
internal fun PainAndGain.commandFight(army: List<Creep>, combatEnemies: List<Creep>, armedEnemies: List<Creep>,
    ...
    // ...А В ДЕБЮТЕ КОНТАКТА — ВПЛОТНУЮ (v287, см. openingNow). Разбор 150+ игр против ●ω<♥♪#6: чей крип умер первым, тот и
    // проиграл — его первым 33-40, наш первым 11-87 (p < 0,0001), — и его первым умирает мили, который врывается и почти
    // не получает лечения, если наш мили стоит рядом: смежность наших мили в первые 15 тиков контакта выше медианы — его
    // крип умирает первым в 23 из 28 игр против 11 из 28 (p = 0,002; повторено в двух других группах версий, p = 0,007 и
    // 0,017), и смежность создаёт шаг нашего мили (178 случаев, 7 — его шагом). Наш мили в дебюте стоит в двух клетках и
    // не бьёт (0,07 врага вплотную), а его стрелки снимают его за 46 тиков. Урок v135 («мили, вошедший в сомкнутый блок,
    // гибнет за 2–3 тика») ограничивает правило дебютом: до первой смерти и не дольше окна размена. Ворота выживания те же —
    // клетка, где крип не переживёт тик, не предлагается; нет вплотную такой — обычная раздача
    for (c in melees.sortedBy { c -> armedEnemies.minOfOrNull { getRange(c, it) } ?: 99 }) {
        if (openingNow && place(c, { p -> armedEnemies.any { getRange(p, it) <= 1 } && ttlAt(c, p.x * 100 + p.y, p) >= 1.0 },
                { p -> danOf(c, p.x * 100 + p.y) })) { openCommit++; continue }
