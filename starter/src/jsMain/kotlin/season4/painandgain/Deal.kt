...
internal class Deal(
    ...
    fun place(c: Creep, wants: (Position) -> Boolean, rank: (Position) -> Double, depth: Int = 0,
        ...
        for ((key, p) in cells) {
            ...
            // ...И СМЕРТЕЛЬНОСТЬ СЧИТАЕТСЯ ТАК ЖЕ, КАК У ТАКТИКА (v524, см. USE_LETHAL_SAME_RULE). Шестой случай
            // класса «одна величина — два правила»: здесь вето ставилось по ВАЛОВОМУ входящему за ОДИН тик, а в
            // оценке клетки тактика (`lethalNow`) — по ЧИСТОМУ, за вычетом нашего лечения, за ДВА тика. Довод, по
            // которому тактик считает чистый, записан там же и целиком применим сюда: штампованное поле не знает
            // про наше лечение, и клетка рядом с тремя лекарями объявлялась смертельной, хотя входящий там
            // перекрыт. Две ветки ведут ОДНОГО крипа в одном тике, и расхождение постоянное, пока лекари живы
            val lethal = if (USE_LETHAL_SAME_RULE)
                InfluenceMap.netDamageAt(p.x, p.y, combatEnemies, living(army)) * 2.0 >= c.hits
                else danOf(c, key) >= c.hits
     ...
     * дальности треть, цель лечит и себя — v266), огонь остальных — по их УЖЕ НАЗНАЧЕННЫМ клеткам (`cellOf`), то
     * есть оценка и исполнение стоят в одном решении одного тика. Это же отличает правку от трёх отвергнутых форм
     * прогноза (v244/v246, v394, v395): те меняли оценку, не меняя того, кто и куда идёт, — и оценка уходила вперёд
     * исполнения.
     *
     * ⚠️ ПЕРВАЯ ФОРМА — РАЗНОСТЬ ОБРЕЗАННЫХ (v483) — ОТВЕРГНУТА СВОИМ ЖЕ ПРИБОРОМ. Вклад считался как
     * `max(0, чужие + мой − лечение) − max(0, чужие − лечение)`, то есть на всякой непробиваемой цели давал РОВНО
     * НОЛЬ. Живой A/B 7+7 против `MetalicaX#17` (отличие — одна константа): счёт 1-6 против 1-6, а прибор ушёл в
     * обратную сторону — выстрелов впустую 14,3 % против 12,8 %, пробивающих назначений 60,8 % против 67,7 %.
     * Причина в самой форме: когда непробиваемы ВСЕ достижимые цели, слагаемое выстрела исчезает из оценки клетки
     * целиком, и расстановка стрелка возвращается к состоянию до v427 — дальность держать становится нечем, стрелок
     * уходит, и сходимость падает вместо роста. Порог в нуле и был дефектом.
     *
     * Действующая форма — та же величина без разрыва: доля огня, проходящая сквозь лечение цели,
     * `мой · (чужие + мой) / (чужие + мой + лечение)`. Цель без лечения отдаёт полный урон, как в v427; цель под
     * тремя лекарями отдаёт малую долю, но не ноль — дальность держать есть чем; и доля РАСТЁТ с числом стволов на
     * цели (60/140 лечения: один ствол 18, два 28, три 34, четыре 38), то есть сходимость поощряется монотонно.
     * Мягкое насыщение — та же форма, что уже принята у притяжения лекаря (v208).
     */
