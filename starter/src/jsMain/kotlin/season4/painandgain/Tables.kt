 ...
 * сама. До v457 это узнавали сравнением тега как строки — `stepTag == "flee"`, `rung == "engage" || rung == "holdMelee" || …`,
 * `stepTag != "order"`: переименовал строку — приоритет предложения молча изменился. Теперь читается `row.mark`; сравнение тега со
 * строковым литералом вне таблицы запрещает линт (`tag_outside_table`, список известных пуст).
 */
internal enum class RowMark {
    NONE,
    /** Шаг выживания (бегство): предложение идёт арбитру с приоритетом SURVIVE. */
    SURVIVE,
    /** Шаг по приказу командира: клетка приказа не закрепляется «опасной», как закрепляется шаг по своей воле. */
    ORDER,
    /** Свободный шаг: терм предложения — ступень лестницы, приоритет решает она же. */
    FREE,
