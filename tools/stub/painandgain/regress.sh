...
# Кат `+heals` в run.mjs существовал, но НИ ОДНА строка гейта его не гоняла; теперь гоняют четыре. Строки стоят в
# GRIND, а не в run: гейт посадки они не сторожат, потому что бот проходит их НЕ ВСЕ — match28 при нынешней настройке
# теряет армию (723:4865), а при `USE_COMMANDER_ALWAYS = true` проходит. Это открытая находка, и ей место в приборе, а
# не в воротах: ворота остаются прежними 131, чтобы вся история замеров была сравнима, а новая форма врага меряется
# рядом и ждёт, когда бот закроет её по существу.
grind map-match13.txt -      brawl+heals
grind map-match20.txt -      brawl+heals
grind map-match28.txt -      brawl+heals
grind map-match35.txt -      screen+focus+blob+heals
