...
исхода, а этот скрипт — логи сценариев целиком. Логи детерминированы, кроме строк `cpu t=` и поля `time=` в `done:`; строки прибора `tac t=` (v252) новые;
...
def lines(p):
    ...
    for l in open(p, encoding='utf-8', errors='replace').read().split('\n'):
        if l.startswith('cpu t=') or 'time=' in l or l.startswith('tac t='):
            continue
        for rx, rep in MASK:
            l = rx.sub(rep, l)
        out.append(l)
