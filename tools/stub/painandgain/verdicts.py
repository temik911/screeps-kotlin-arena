...
удаления проверяется: всё, что вычеркнуто, должно найтись в docs.
  ...
  emit   [--code PATH] [--out PATH]      печатает приложение (по умолчанию docs/pain-and-gain-verdicts.md): раздел на
                                         ...
                                         Идемпотентно: блок, чьи первые 60 нормализованных символов уже есть в docs,
                                         пропускается, так что запуск после частичного удаления дописывает только новое.
...
def emit(code_path, out_path):
    ...
    def present(text_lines):
        head = norm(' '.join(text_lines))[:60]
        return bool(head) and head in known
