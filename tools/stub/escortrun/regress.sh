...
# left to V8's 4 GB default limit its process grows to 85 MB. A 2 MB semi-space and an old-space limit of 192 MB keep it
...
run() { # $1 = map file or -, $2 = START or -, $3 = scenario, $4.. = extra env (KEY=VALUE)
  ...
  env+=("$@")
  local raw; raw=$(env "${env[@]}" "$NODE" --import ./register.mjs run.mjs 2000 "$sc" 2>&1)
