...
def push_zip(c, data):
    ...
    c.eval("window.__parts = []; 1")
    for i in range(0, len(b64), 400_000):
        c.eval(f"window.__parts.push({json.dumps(b64[i:i + 400_000])}); 1")
    return c.eval("""(() => {
      const bin = atob(window.__parts.join('')); window.__parts = null;
      const arr = new Uint8Array(bin.length);
      for (let i = 0; i < bin.length; i++) arr[i] = bin.charCodeAt(i);
      window.__code = new Blob([arr], {type: 'application/zip'});
      return window.__code.size;
    })()""")
...
def start(c, arena_id):
    return c.json_eval(f"""(async () => {{
      ...
      fd.append('code', window.__code, 'code.zip');
