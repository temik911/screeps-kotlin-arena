...
5. **`main` collects finished work, not every build — land by the batch, not by the version.** A session lands when there is a body of work worth landing (a defect closed, a feature that plays, a run of versions that belong together), whenever a change is one other sessions build on (see below), and always before the session ends. Landing once per version is waste: the gate runs **every** arena's stub, and the landing session's own arena is a rounding error in it — measured 05.09.2026, `escortrun` 9 s, `spawnandswamp` 78 s, `painandgain` 212 s, build 1 s, so one landing costs about five minutes of which 97% is other people's scenarios. Five landings in one evening cost half an hour and protected nothing that one landing would not have.
   **A live match does NOT require landing.** It requires the cheap half of what the old rule was really after: the code being played is **committed on your branch** (nothing uncommitted in the tree — the client runs your worktree's build, so an uncommitted tree means a match nobody can reproduce), and **the bot names its version in the first log line** (`hello … vN` plus the `tuning:` signature). That pair is what ties a match log back to code; being in `main` never was — a log referring to a `main` commit and a log referring to a branch commit are equally findable, and the version line is what you actually read. Half-done work stays on the branch either way.
   ⚠️ **The exception, and it is not optional: land at once, without waiting for a batch, anything other sessions build on** — `types/`, `tools/`, the Gradle files, `CLAUDE.md` and these rules. There the five minutes buy something real: until it is in `main`, every other session either lacks it or writes it a second time (the season 4 effect modifiers were committed twice, identically, by two sessions).
   Land with `tools/land.sh [--tag <arena>-vN] [--no-push]` from your worktree: it refuses a branch that is not rebased on `main` or has uncommitted changes, runs `./gradlew build`, runs every `tools/stub/*/regress.sh` (every scenario line must carry the arena's pass marker — `PASS`, or `ENEMY SPAWN DESTROYED` where there are spawns — and zero errors), fast-forwards `main` (`git fetch . <branch>:main`, or `git -C <root> merge --ff-only` when the root is on `main` and clean), tags the landed version if asked, and pushes `main` and the tag to `origin` — pushing `main` is part of landing, so the current bots are never only on one disk; a session branch is pushed only when the operator asks. If `main` moved meanwhile, `land.sh` stops: `git rebase main`, rebuild, rerun, land again. Never land a tree that does not build: every session's `./gradlew build` compiles every package, so a broken package in `main` stops all of them. `origin`'s push URL is SSH.
...
- **Live matches can be started from here — `tools/play.py`, one session's matches at a time, no UI.** The client authenticates with a Steam session ticket (`ipc-main::get-steam-session-ticket` → `POST /api/auth/login`), so the API cannot be called from outside it; but it does not disable remote debugging, so launched as `cd "<app>/Contents" && SteamAppId=1137320 ./MacOS/screeps_arena --remote-debugging-port=9222` (Steam running) its page can be driven over CDP, and `fetch` from that page carries the session. `tools/arena_cdp.py` is the protocol client (stdlib websocket); `tools/play.py --list` shows the season's arenas with ids, script folders and free slots, `tools/play.py <arena> -n 20 --stop-on-defeat --logs runs/` plays a series and writes each match's console. The payload is a zip of that arena's client folder (everything but `typings/` and `jsconfig.json`), whose `node_modules` symlink points at **your** worktree build — so every session plays its own bot, and since nothing navigates the UI, sessions do not collide: the client runs up to three series at once (`AVAILABLE_PLAY_SESSION_COUNT = 3`) and the server keeps one match slot per arena. Rating matches move the rating, so land the version first (rule 5) — the match log then refers to a commit that is in `main`.
- **Full match replays — both sides' attacks, heals and positions per tick — are readable here: `tools/replay.py`** (summary / silent / focus / trace of a fight window). The replay itself comes from `arukuka/screeps-arena-tools` (GPL, lives outside the repository; the docstring says how to build it and fetch a match), which drives the running client over its Node inspector — the client must have been started by its absolute path (`tools/arena_cdp.py`'s hint does), or the tool reports it as not running. This is what the console log cannot show: the enemy's intents.

- **One command per match — `tools/autopsy.py <id|prefix|log>`** joins the two above into one report: the outcome's form, the opponent's form by the replay track, the first contact's geometry, the fight decomposed (expected damage and healing, uptime per role, adjacency, deaths), the bot's nets and its `why` decision trace, and a DIAGNOSIS list of rules with the numbers that crossed them; `--json` for aggregation, `--fetch` to pull a missing replay through the running client. Replays live in `~/ScreepsArena/replays/`, arukuka's tool in `~/ScreepsArena/screeps-arena-tools/` (not in a job's temporary folder — that is deleted with the job).
