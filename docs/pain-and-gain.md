...
**Где бот.** В `main` v674 (тег `pain-and-gain-v674`), рейтинг 1 746, первое место (второй — ricardo, 1 525);
рейтинговые серии 26.09: v665 — 19 из 20, v669 — 18 из 20, v671 — 19 из 20, v674 — 17 из 20; Fame — 9 из 10. Игра по
умолчанию — v551, поверх неё раскладки по почерку соперника, каждая за своим признаком (защёлка в `Memory`, печать в
`raid=`):
...
- `keeper` — хранитель отрыва, против него пары (v646); `chemo` — признак Chemoautotroph (v638), тройкам не мешает
  ...
  «сперва своя половина» — только среди берущихся флагов (v666); размен с идущим на нас кулаком держится до первого боя
  (v667–v669); качающийся разведчик идёт в обход зоны бегства (v674).
...
1. **MetalicaX#13 — главное поражение серий v665–v674** (с v640 — 52–25): аннигиляция за 200–500 тиков в раннем бою.
   Ранний бой по `engageLost` — часть сценария и против него (106 побед из 108 с ним), удержание v667–v669 здесь не
   нужно (`engheld=0`): бой проигран в самом размене, первые 6–15 тиков. Замечено: против #13 со старта (13,10) — 77 %
   побед, с (85,88) — 57 %. Это отдельное исследование размена, не правка правила.
