...
**Где бот.** В `main` v685 (тег `pain-and-gain-v685`), рейтинг 1 762, первое место (второй — ricardo, 1 525);
рейтинговые серии 26.09: v665 — 19 из 20, v669 — 18 из 20, v671 — 19 из 20, v674 — 17 из 20, v679 — 19 из 20, v682 — 18
из 20; Fame — 9 из 10. Игра по умолчанию — v551, поверх неё раскладки по почерку соперника, каждая за своим признаком
(защёлка в `Memory`, печать в `raid=`):
...
- `frag` — ricardo: разведчик на H к t=100 и армия раздроблена — три группы по 2+ или четыре группы с одиночками (v665),
  ...
  обходит клетки наших флагов, раздетого товарища не ждут (v681);
- `flip` — けろびー#20 и его семья #17–#19: оба H его вооружёнными к t=90 и два наших флага перевёрнуты к t=150 — те же
  ...
  дешёвый (v670); пост бросается только сверх нормы ⌊живые / 3⌋ (v671); «H до пары ради D5» отвергнуто (v683 → v685:
  подкрепление по одному гибнет в его штурме 9–11);
...
1. **けろびー#22 — пол по цене (v684) живьём.** Его нет в тестовом списке; при следующей встрече (рейтинг) смотреть
   `floorval=` и `raid=hold` в дебюте: стрелки должны стоять на H к t≈160, а не на A/R.
2. **けろびー#20 — D5 при `flip`.** Штурмы D5 по 9–11 его бойцов: подкрепление по одному гибнет (v683 отвергнуто). Открытые
   направления: отряд D5 с милей с самого начала (без мили 3–2, с милей 9–0, p≈0,11); подкрепление целым отрядом между
   штурмами, а не по одному во время; из угла (13,10) против него 4–4, из (85,88) 9–0.
3. **MetalicaX#13** — поражения решаются после контакта (его четыре мили вплотную за ≤5 тиков — 43 % поражений против 5
   %), признака до размена нет (разбор v680, 87 игр); местность исходов не делит (v682, 95 игр, `bog=` до боя 0 в 19
   играх из 20). Дальше — механика ближнего боя; прикрытие мили отвергнуто дважды (v647/v651).
4. Когда соперники обновят ботов — рейтинговая серия из 20 и разбор ВСЕХ поражений субагентами Opus по `имя#версия`; у
   новой версии первым делом проверить `raid=`. Ещё не встречались: dd#5 (третье место), MetalicaX#21, MBFishhh#19,
   ricardo#27. Отвергнутое — в `Tuning.kt` и `history-v4xx.md`; перед новой правкой спросить `tools/know.py`.
