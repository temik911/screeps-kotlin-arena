...
## Состояние и что дальше (26.09.2026, вечер)

**Где бот.** В `main` v679 (тег `pain-and-gain-v679`), рейтинг 1 760, первое место (второй — ricardo, 1 525);
рейтинговые серии 26.09: v665 — 19 из 20, v669 — 18 из 20, v671 — 19 из 20, v674 — 17 из 20, v679 — 19 из 20; Fame — 9
из 10. Игра по умолчанию — v551, поверх неё раскладки по почерку соперника, каждая за своим признаком (защёлка в
`Memory`, печать в `raid=`):
...
- `frag` — ricardo: разведчик на H к t=100 и армия раздроблена — три группы по 2+ или четыре группы с одиночками (v665),
  ...
  (v649), отряд меньше тройки сперва берёт лишних (v662), закреплён только держатель клетки флага (v660);
- `flip` — けろびー#20 и его семья #17–#19: оба H его вооружёнными к t=90 и два наших флага перевёрнуты к t=150 — те же
  ...
  дешёвый (v670); пост бросается только сверх нормы ⌊живые / 3⌋ (v671);
- `keeper` — хранитель отрыва, против него пары (v646); `chemo` — признак Chemoautotroph (v638), тройкам не мешает
  ...
  качающийся разведчик идёт в обход зоны бегства (v674).
...
1. **MetalicaX#13** — после v675/v679: тестовые руки 10 из 12, серия v679 — 3 из 4 (с v640 было 52–25). Нужен прибор:
   включался ли порядок рядов и сколько наших стволов достают его в тик первого размена (разбор v674: у кого больше — 83
   % против 50 %); затем разбор поражения 6ab7f7f0 (199 тиков).
2. Открытое без правки: `cohere` держит гарнизонного крипа сколько угодно долго, и под огнём (150 крипо-тиков в
   поражении от ricardo#20 против 0–27 в победах); «с нашим телом на клетке не берёт» у けろびー#20 верно для H, но не для
   D5 — D5 он штурмует сменой бойцов.
3. Когда соперники обновят ботов — рейтинговая серия из 20 и разбор ВСЕХ поражений субагентами Opus по `имя#версия`; у
   новой версии первым делом проверить `raid=` (ложный признак включает чужую раскладку, новый почерк может не поднять
   нужный). Свежие версии в таблице: ricardo#27, けろびー#24, MetalicaX#21, dd#5 (новый, третье место) — ещё не встречались.
4. Отвергнутое на этом поле — v647/v651 (прикрытие мили), v653 (изготовка до контакта), v655 (вето R/A у ждущей массы) —
   в `Tuning.kt` и `history-v4xx.md`; перед новой правкой спросить `tools/know.py`.
