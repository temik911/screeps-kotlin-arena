...
**Где бот.** В `main` v660 (тег `pain-and-gain-v660`), рейтинг 1 717, первое место; последняя рейтинговая серия — 18 из
20. Цель оператора «90 %, затем 95 % против поля» закрыта по его порогу «18+ из 20»; работа остановлена до того, как
соперники обновят ботов. Игра по умолчанию — v551, поверх неё раскладки по почерку соперника, каждая за своим признаком
(защёлка в `Memory`, печать в `raid=`):

- `tourer` — объездчик けろびー#22/#23 (v634; тактика v621: шар 4M3H на D5, стрелки на H, пол флагов);
- `frag` — ricardo: разведчик на H к t=100 и армия дробится на 3+ группы по 2+ — стоящие тройки на D5, H, H, A/R (v644,
  `Garrisons.farmerTriples`), слияние бросает дешёвый пост (v649), закреплён только держатель клетки флага (v660);
- `flip` — けろびー#20 и его семья #17–#19: оба H взяты его вооружёнными к t=90 и два наших флага перевёрнуты к t=150 — те
  же тройки (v657, `Garrisons.flipperRule`);
...
tools/play.py pain-and-gain --ab main HEAD --dry`, застой и раскладку на настоящей карте — призрак записанной игры
(`REPLAY=<id>.replay.json.gz … run.mjs 2000 ghost`).

**Что дальше — когда соперники обновят ботов:**

1. Рейтинговая серия из 20 (`python3 tools/play.py pain-and-gain -n 20 --logs runs/`), затем разбор всех поражений
   субагентами Opus по боту `имя#версия` (`match-log.py list`, `autopsy.py`, реплей — `match-log.py replay`). У новой
   версии соперника первым делом проверить `raid=`: ложный признак включает чужую раскладку, а новый почерк может не
   поднять нужный.
2. **けろびー#20 — оба поражения серии v660** (6ab56b00, 6ab56b8b): без боя теряем один H, доска 9:16, а v659 выигрывала у
   него 5 из 5, держа D5 и оба H (13:12, запас в очко). Первый шаг — призраки обеих игр на v659 и на v660: если H держит
   только v659, сузить `USE_PIN_FLAG_ONLY` до признака `frag` (v660 нужна лишь против застоя отряда у ricardo). Если
   держат обе — выяснить по реплею, почему держатель H уходит, когда гонка отрывает бегунов (`detach … race=true` на
   t=277 и 281), и держать оба H при тройках.
3. **MetalicaX (около 70 %).** Разбор поражения v659 от #13: наш захват R/A до первого боя снимает бой, начатый по
   `engageLost` (`Strategist.kt:180`), — армия 15 тиков стоит в досягаемости его блока, а пост HOLD уводит стрелков к
   дальнему флагу; с этим рисунком против MetalicaX 43 % побед, без него 71 %. Кандидат — захват флага не снимает
   начатый бой, пока его сомкнутая армия в `BRACE_RANGE`; похожее вето v655 упало на `match31:camp`, это первая строка
   гейта для проверки.
4. Отвергнутое на этом поле — v647/v651 (прикрытие мили), v653 (изготовка до контакта), v655 (вето R/A у ждущей массы) —
   описано в `Tuning.kt` и `history-v4xx.md`; перед новой правкой спросить `tools/know.py`.
