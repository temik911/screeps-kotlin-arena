...
**Цена правки — что тронуть, чтобы добавить…** (пересчитано на v460 тем же способом, что раздел 1 плана
...
| # | действие | до плана (v452) | сейчас (v460) — прослеженный пример | что держит |
|---|---|---|---|---|
...
| 5 | новое правило постуры с новым входом | 4 | **4 — НЕ СНИЖЕНО**: строка `POSTURE_RULES`, поле `Strategist.Inputs`, `val evade` в `StrategyDecide`, аргумент `evade = evade`. План противоречит себе (п. 4 этапа 3 оставляет `Inputs` списком, приёмка этапа 4 требует двух мест) — вопрос оператору, вариант А: `Inputs` — носитель | — |
...
| 9 | найти место правки стратегии | одна функция на 690 строк и 96 локальных | подстадия с именем и своим выходом: 8 у стратега, 4 у мер, 6 у целей, 5 у стойки, 5 у гонки, 2 у бегунов | `tools/cutwidth.py`, подстадии (v456–v457) |
...
**Строки гейта, которые не дают цене вырасти обратно:** `lint` (23 правила; списки известных нарушений только убывают — на v460:
`plumbing` 16, `single_writer` 6, остальные пусты), `graph` (рёбра вопреки уровням — 0, известных 0), `impure` (перебор замыслов
