...
## Состояние и что дальше (26.09.2026, ночь)

**Где бот.** В `main` v690 (тег `pain-and-gain-v690`), рейтинг 1 737, первое место (второй — ricardo, 1 525);
рейтинговые серии 26.09: v665 — 19 из 20, v669 — 18 из 20, v671 — 19 из 20, v674 — 17 из 20, v679 — 19 из 20, v682 — 18
из 20, v685 — 16 из 20 (пик рейтинга 1 787); Fame — 9 из 10. Игра по умолчанию — v551, поверх неё раскладки по почерку
соперника, каждая за своим признаком (защёлка в `Memory`, печать в `raid=`):

- `tourer` — объездчик けろびー#22/#23 (v634; тактика v621: шар 4M3H на D5, стрелки на H, пол флагов); допуск смещения пары
  разведчиков ±2 (v663); пол флагов считает по цене, а не штуками: дешёвый флаг, взятый разведчиком, не занимает место H
  (v684), а группа стрелков свой взятый флаг ради флага той же цены не бросает (v687) — тестовые руки v688 против #22 6
  из 8;
- `frag` — ricardo: разведчик на H к t=100 и армия раздроблена — три группы по 2+ или четыре группы с одиночками (v665),
  5 срезов (v662) — стоящие тройки на D5, H, H, A/R (v644, `Garrisons.farmerTriples`); слияние бросает дешёвый пост
  (v649), отряд меньше тройки сперва берёт лишних (v662), закреплён только держатель клетки флага (v660); марш к посту
  обходит клетки наших флагов, раздетого товарища не ждут (v681); срез «четыре группы» не считается при кулаке больше
  двух троек (v689, против MBFishhh#1);
- `flip` — けろびー#20 и его семья #17–#19: оба H его вооружёнными к t=90 и два наших флага перевёрнуты к t=150 — те же
  тройки (v657), но пост H не бросается целиком, пара на своём флаге стоит (v661) и пара не уходит с дорогого поста в
  дешёвый (v670); пост бросается только сверх нормы ⌊живые / 3⌋ (v671); на D5 трое вооружённых, лекари по одному на
  пост, нехватка стволов на D5 берёт дешёвый пост целиком (v688: он нападает на D5 только при двух стволах и меньше —
  тестовые руки 8 из 8); «H до пары ради D5» отвергнуто (v683 → v685);
- `keeper` — хранитель отрыва, против него пары (v646); `chemo` — признак Chemoautotroph (v638), тройкам не мешает
  (v659). Режим пар по расколу его армии открывается, только если раскол держится 40 тиков (v664, против Katterton);
  «сперва своя половина» — только среди берущихся флагов (v666); размен с идущим на нас кулаком держится в окне первого
  столкновения (v675); против идущего на нашу половину кулака до первого боя стрелки стоят первым рядом (v679,
  срабатывает 0–7 тиков за игру — `rfirst=`); качающийся разведчик идёт в обход зоны бегства (v674).

Всё, что делается против одного бота, — только за таким признаком; тождество с прежней игрой проверяет `python3
tools/play.py pain-and-gain --ab main HEAD --dry`, механику на настоящей карте — призрак записанной игры
(`REPLAY=<id>.replay.json.gz … run.mjs 2000 ghost`), но против соперника, который РЕАГИРУЕТ на нас (переворачивающий
#20), призрак не судья — там только тестовые руки. Приборы v680–v684: `rfirst=`, `clashg=` (стволы обеих сторон в первый
тик досягаемости и в тик первого размена), `bog=` (армия на болоте до боя / после), `floorval=` (пол, пропущенный
ценой). Простой марша — «центр армии не отходил дальше клетки» (v690).

**Что дальше:**

1. **MBFishhh#1 (пятый в таблице) — главная дыра: v685 0 из 9, v690 1 из 8.** Его глыба 2M5R3H держит дистанцию 5–8 и не
   дерётся, одиночки и разведчики держат флаги; наша армия в ANNIHILATE без цели (`objnone=annihilate` 550–1 180 тиков),
   его очки во многом с пустых клеток. Следующий заход — постура против стаи, которая держит дистанцию и не дерётся
   (цель — флаги при целой армии), и свободный разведчик на его пустые клетки; `frag` в 6ab82d29 встал другим путём
   признака — разобрать.
2. **ricardo — обход его станции.** Тройка без мили гибнет на марше к дальнему H сквозь его станцию у A (4 игры из 24,
   поражение 6ab819e9); кандидат — поле марша в обход его вооружённых у его флагов, если они сильнее отряда (прецедент
   USE_POST_AVOID, v629, отдельно не мерился).
3. **けろびー#22 — перехват шара** без стрелков по дороге к D5 (6ab81a65, разгром на t=109–150); лечение уходом отвергнуто
   (v624), проверка пути к цели бегства (`routeSafe`) не пробовалась.
4. **MetalicaX#13** — поражения решаются после контакта (его четыре мили вплотную за ≤5 тиков), признака до размена нет
   (v680); местность исходов не делит (v682).
5. Когда соперники обновят ботов — рейтинговая серия из 20 и разбор ВСЕХ поражений субагентами Opus по `имя#версия`; ещё
   не встречались dd#5, MetalicaX#21, ricardo#27. Отвергнутое — в `Tuning.kt` и `history-v4xx.md`; перед новой правкой
   спросить `tools/know.py`.
...
runArmy:  updateKeepers → ArmyMeasures (4 группы мер) → ArmyStrategy (девять подстадий; Strategist.decide) → ArmyTargets (6) → ArmyStance (5) → armyBlock
...
**Цена правки — что тронуть, чтобы добавить…** (пересчитано на v461 тем же способом, что раздел 1 плана
`docs/pain-and-gain-architecture-2.md`: прослежен существующий пример, считаются места, которые механизм ОБЯЗЫВАЕТ тронуть, —
объявление, параметр, аргумент, сброс, печать, маска; места, где величина просто используется, не считаются).

| # | действие | до плана (v452) | сейчас (v461) — прослеженный пример | что держит |
|---|---|---|---|---|
| 1 | новый факт хода для строки лестницы | 3 места | **1**: `val leashed = …` в теле `class Turn` (`Tactician.kt`); строка читает `{ leashed }` | носитель (v456) |
| 2 | новый факт шага | 3 | **1**: `val mustFlee = …` в теле `class Stride` | носитель |
| 3 | новая величина от стадии к стадии | 3; через синглтон — около 6 в трёх файлах | **1**: `val contact = …` в теле подстадии (`MeasuresFight`); у трёх стадий-функций, отдающих через объект-владельца (`readSignals` → `Signals`, `healerWall` → `Wall`, огонь → `FireBook`), — 2 (поле владельца и запись) | носители, `Prev`, владельцы (v456, v459) |
| 4 | новый счётчик в строке `t=` | 4 места в 3 файлах; счётчик прохода раздачи — 6 в 4 | **2**: объявление `val rotOut = Gauges.counter("rot")` у считающего и имя `"rot"` в раскладке `T_LINE`; новая ЧАСТЬ существующего поля — 1. Счётчик прохода раздачи: `val fallReach = g.counter("fall")` в `DealRecord` и имя в раскладке; слияние, сброс, починка, печать и маска оракула — по реестру | реестр `Gauges` (v455), автомаска `logdiff.py` (v453) |
| 5 | новое правило постуры с новым входом | 4 | **2**: поле `val evade = …` в теле подстадии `StrategyInputs` и строка `Row("evade", { evade })` в `POSTURE_RULES` — предмет таблиц `Strategist.decide` и есть эта подстадия (до v461 — 4: ещё поле списка `Strategist.Inputs` и аргумент `evade = evade`) | носитель входов (v461) |
| 6 | новая память между тиками | 2 + чистка мёртвых id руками в другом файле | **1**: `val x = perCreepMap<…>()` в `Memory` — мёртвых снимает общая `Memory.prune`; не по id бойца — 1 (поле владельца) | `Memory.prune` (v459) |
| 7 | новый проход раздачи, новые ворота, новая строка лестницы, новая константа | 1–2 | 1–2, без изменений (метод и строка списка; строка списка) | таблицы (v444–v445) |
| 8 | понять, почему крип сделал X | тег строки в логе и один ручной прибор одного факта | причина — у каждого из 37 сложных фактов (`whynot t=`, срез `tools/series.py whynot`) и общая трасса `trace t=` в окне тиков; новому факту — имена конъюнктов в месте вызова | `Why` (v457–v458) |
| 9 | найти место правки стратегии | одна функция на 690 строк и 96 локальных | подстадия с именем и своим выходом: 9 у стратега, 4 у мер, 6 у целей, 5 у стойки, 5 у гонки, 2 у бегунов | `tools/cutwidth.py`, подстадии (v456–v457) |
| 10 | проверить правку на стенде | полный гейт — 139 сценариев | `identity.sh <эталон> --rows gate.majority` — 4 сценария, `pass.straggler` — 5, `rung.retreat` — 12; полный гейт — перед посадкой | `gategap.py`, быстрый цикл (v453) |
| 11 | проверить правку живьём | две сборки и серия руками, 3–4 часа внимания | одна команда: `tools/play.py pain-and-gain --ab <refA> <refB> --test 'Bot#N' -n 8` | `play.py --ab` (v453) |
| 12 | узнать, пробовали ли это | `grep` по 1,41 МБ | `tools/know.py <тег / факт / константа>` | разрез истории, `know.py` (v453) |
| 13 | не сломать прежнее решение о порядке | комментарий между строками таблицы | запись с причиной в `tools/stub/painandgain/order.txt` (31 запись: лестница, шаг, ворота, наступление, проходы раздачи), сверяет линт `table_order` | линт (v453, v460) |

**Строки гейта, которые не дают цене вырасти обратно:** `lint` (23 правила; списки известных нарушений только убывают — на v461:
`plumbing` 5 — именованный аргумент функции, чьё имя совпало с локальной, — остальные пусты), `graph` (рёбра вопреки уровням — 0, известных 0), `impure` (перебор замыслов
командира ничего не пишет в мир — 0 на всём гейте).
...
- **Носитель: поле объявлено там, где вычислено** (v456, второй шаг архитектуры, этап 3). Стадия армии — КЛАСС, чьё тело и есть
  ...
  `!= null`, как сужал локальную: там пишется `x!!` с комментарием, где непустота проверена. Входы решателя постуры —
  тоже носитель (v461, слово оператора): таблицы `Strategist.decide` читают поля подстадии `StrategyInputs`, списка
  `Strategist.Inputs` нет — цена названа: `decide` больше не строится без игровых объектов; `PushCase` читает меры и пачки из носителей. Пустых `…Out` нет (v459).
- **Отряды — `object Squads`** (v460, этап 6.5; `Squads.kt`, уровень 1): восемь реестров членства — его прямые поля, и записи трёх из
  них (`cmdDetach`, `detachedIds`, `runnerFlag`) — ТОЛЬКО его операциями: `detach(id, flagId)` (командир: член набора И его флаг —
  одной операцией), `detach(id)` (стратег), `recallAll(источник)`, `recallAllBut(keep)`, `recall`, `keepDetached`, `assign` /
  `unassign` / `unassignAll` / `keepAssigned`. Два источника (`STRATEGIST`, `COMMANDER`) остаются двумя наборами — сведение к
  одному владельцу на крипа меняет поведение. Одиннадцать неявных инвариантов (членство действует со следующего тика, порядок
  стадий, недостроенный `cmdDetach` после ранних выходов гонки…) записаны в KDoc объекта. Приборы: `squads=` — размеры реестров,
  `dbl=` — крипо-тики двойного членства по парам (его не стережёт ничто; на гейте оно случается: хранитель + командир в 49
  сценариях из 140, гарнизон + хранитель в 12, отряд стратега + командир в 3). Новое место записи членства — операция `Squads`,
  а не прямая запись: иначе правило одного писателя упадёт.
- **Перебор замыслов командира чист** (v460, этап 6.7): проба замысла считает в свою запись (`DealRecord`), в мир уходит только
  выбранная (`publishDeal`). Владельцы общих полей считают свои записи (`InfluenceMap.publishedWrites`, снимок приказов), командир
  сверяет до и после перебора — поле `impure=` строки `t=`; на всём гейте оно обязано быть нулём (строка `impure` в `regress.sh`
  блокирует посадку, как `lint` и `graph`). Новая оценка внутри раздачи, которая пишет в общее состояние, упадёт здесь.
- **Длинная стадия — цепочка подстадий-носителей** (v456–v457, этап 4): `ArmyStrategy` — список из девяти вызовов (`StrategyPacks` →
  `StrategyThresholds` → `StrategyDetach` → `StrategyPush` → `StrategyContact` → `StrategyObjective` → `StrategyInputs` →
  `StrategyDecide` → `StrategyThreats`), у каждой свой выход; параметры подстадии — носители предыдущих, и читает она их квалифицированно
...
- [`history-v4xx.md`](pain-and-gain/history-v4xx.md) — v401 и далее: мера боя, архитектура решений v441–v447, доработки
  А–Д v448–v452, второй шаг архитектуры с v453, против поля v626–v660 (раскладки по почерку соперника, тройки); **новые
  абзацы версий пишутся сюда**, 96 разделов на 24.09.2026
