...
## Состояние и что дальше (26.09.2026)

**Где бот.** В `main` v665 (тег `pain-and-gain-v665`), рейтинг 1 722, первое место (второй — ricardo, 1 525); последняя
рейтинговая серия — 19 из 20, Fame 26.09 — 9 из 10. Игра по умолчанию — v551, поверх неё раскладки по почерку соперника,
каждая за своим признаком (защёлка в `Memory`, печать в `raid=`):
...
- `flip` — けろびー#20 и его семья #17–#19: оба H его вооружёнными к t=90 и два наших флага перевёрнуты к t=150 — те же
  тройки (v657), но пост H не бросается целиком и пара на своём флаге стоит (v661);
...
1. **MetalicaX — единственное поражение серии v665** (#13, 6ab7c0c8, аннигиляция за 200 тиков; около 70 % против него).
   Разбор v659: наш захват R/A до первого боя снимает бой, начатый по `engageLost` (`Strategist.kt:180`), армия стоит в
   досягаемости его блока, пост HOLD уводит стрелков к дальнему флагу; с этим рисунком 43 % побед, без него 71 %.
   Кандидат — захват флага не снимает начатый бой, пока его сомкнутая армия в `BRACE_RANGE`; похожее вето v655 упало на
   `match31:camp`.
2. **Katterton — тупик фильтра «сперва своя половина»** (`Strategist.kt:1743`): в режиме пар список захватов пуст, когда
   оба флага нашей половины под его телом или кулаком, а чужой H 1 500 тиков пуст. v664 снимает причину (режим пар не
   открывается от мгновенного раскола), сам фильтр не тронут.
