...
- **Функция — расширение `PainAndGain` только тогда, когда ей нужен приёмник** (v454, второй шаг архитектуры, этап 1): она
  сама или те, кого она зовёт, читают член объекта. Иначе это обычная функция файла стадии, и имя в её теле разрешается по
  двум областям (локальная → верх пакета), а не по четырём (локальная → вход сегмента → член объекта → верх пакета). Держит
  правило линта `needless_receiver` (список известных пуст): расширение без нужды — FAIL. Обёртки фактов (`hasWeapon(c)`,
  `healerOnly(c)`…), арифметика тела и скорости, поля потоков, геометрия — обычные функции. Пока у объекта остаются члены
